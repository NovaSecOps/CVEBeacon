...
"$ROOT/.venv/bin/python" -m PyInstaller --noconfirm --clean --onedir --name cvebeacon --collect-data cvebeacon --distpath "$ROOT/dist/onedir" --workpath "$ROOT/build/onedir" --specpath "$ROOT/build" --paths "$ROOT/src" "$ROOT/scripts/cvebeacon_entry.py"

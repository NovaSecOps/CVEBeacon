...
The cross-platform workflow runs deterministic tests on Windows Server 2025 and Ubuntu 24.04 with CPython 3.11, 3.12, 3.13, and 3.14. A configured matrix is a validation target; consult the completed workflow results before treating a platform as verified.
...
After all test jobs pass, native packaging jobs build both one-folder and one-file applications with Python 3.12. They run the same isolated smoke checks and inspect the archives for operational data and embedded local application paths. Successful jobs retain platform-specific ZIP or tar archives for 14 days as workflow artifacts. The tar archive preserves Linux executable permissions. These artifacts are validation builds, not published releases.
